Rebol [
    ...
    notes: --[
     A. This test supports Firebird even though it is non-standard, because
     ...
     B. Some databases (notably MySQL and SQLite) allow backticks when
     ...
     C. Table names cannot be parameterized in ODBC statements, so instead of
        $table-name we have to use $[table-name] to splice the name into the
        statement before sending it to ODBC.  The rule for doing this type of
        splice is that strings are not allowed...only LOADed data.  This is
        true also if we are injecting arbitrary SQL code (like AUTO_INCREMENT)
        where a BLOCK! must be used.

        As a last resort, if you need to inject a string value then use the
        $[<!> expr] syntax to force the expression to do the unsafe thing.
    ]--
]
...
for-each [label sqltype content] tables [
    let table-name: join word! ["test_" label]  ; [C]
